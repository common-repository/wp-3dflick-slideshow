...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ flc_upload_dir+"/"+file.name + '&task=flc_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	flc_refresh_images_table(flc_current_album_id);
...
function flc_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=flc_get_albums_table', function(res)
	{
		flc_set_data_tables();
		if (res.status == 'ok') 
		{
...
function flc_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=flc_get_albums_images_table&album_id='+album_id, function(res)
	{
		flc_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'flc_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'flc_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'flc_reorder_image' )
		{
			flc_refresh_images_table(album_id);
		}
		if( task == 'flc_reorder_album' )
			flc_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function flc_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', flc_submit_bulk);
...
function flc_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'flc_bulk_delete_albums' || task == 'flc_bulk_enable_albums' || task == 'flc_bulk_disable_albums')
	{
	...
	else if( task == 'flc_bulk_delete_images' || task == 'flc_bulk_enable_images' || task == 'flc_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = flc_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: flc_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: flc_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: flc_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: flc_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : flc_url+"/js/swfupload/js/swfupload.swf",
				upload_url: flc_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":flc_upload_dir, "secw":flc_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: flc_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.flc_delete_album').live('click', flc_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', flc_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', flc_disable);
